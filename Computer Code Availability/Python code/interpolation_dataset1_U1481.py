...
patch_output     = config.directory_output_images_U1481
...
def export_interp_and_csv(data,name_csv_export):
    ...
    for fc in range(0,len(df3)):
        ...
        for fcc in range(0,len(cols_alter)):
            if ( ( ((cols_alter[fcc] == 'value_1_gra') or (cols_alter[fcc] == 'value_1_ms') or (cols_alter[fcc] == 'value_1_pwl') or (cols_alter[fcc] == 'value_1_rgb') or (cols_alter[fcc] == 'value_2_rgb') or (cols_alter[fcc] == 'value_3_rgb')) and (df3["Core"][fc] == 16) ) or
                ( ((cols_alter[fcc] == 'value_1_pwl') or (cols_alter[fcc] == 'value_1_ngr')
                or (cols_alter[fcc] == 'value_1_rsc') or (cols_alter[fcc] == 'value_2_rsc') or (cols_alter[fcc] == 'value_3_rsc') or (cols_alter[fcc] == 'value_1_rgb') or (cols_alter[fcc] == 'value_2_rgb') or (cols_alter[fcc] == 'value_3_rgb') ) and (df3["Core"][fc] == 18) ) or
                ( (cols_alter[fcc] == 'value_1_pwl') and (df3["Core"][fc] == 2) ) ):
...
core = np.arange(start=1, stop=22, step=1) #adjust according to hole interval - core not used: 22R
typee = ['R']
                            ...
                            for FnLitho in range (0,len(files_output_litho)):   
                                ...
                                for fp in range (0,len(files_properties)):
                                    ...
                                    if (files_properties[fp] == nome) and (log != nome):
                                        ...
                                        '''filePPP0 = filePPP_[filePPP_["Prop"] == prop[0]]
                                        filePPP0 = filePPP0.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")
                                        
                                        filePPP1 = filePPP_[filePPP_["Prop"] == prop[1]]
                                        filePPP1 = filePPP1.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")
                                        
                                        filePPP2 = filePPP_[filePPP_["Prop"] == prop[2]]
                                        filePPP2 = filePPP2.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")

                                        filePPP3 = filePPP_[filePPP_["Prop"] == prop[3]]
                                        filePPP3 = filePPP3.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")
                                        
                                        filePPP4 = filePPP_[filePPP_["Prop"] == prop[4]]
                                        filePPP4 = filePPP4.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")

                                        filePPP5 = filePPP_[filePPP_["Prop"] == prop[5]]
                                        filePPP5 = filePPP5.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")

                                        filePPP6 = filePPP_[filePPP_["Prop"] == prop[6]]
                                        filePPP6 = filePPP6.drop_duplicates(subset=['Offset'])
                                        filePPP_ = pd.read_csv(patch_properties+files_properties[fp],sep=",")

                                        join_fileP = [filePPP0 , filePPP1 , filePPP2 , filePPP3 , filePPP4 , filePPP5 , filePPP6]
                                        filePPP = pd.concat(join_fileP)'''
                                        

                                        
                                        file_json_csv = pd.read_csv(config.file_json_csv_U1481,sep=",")
                                            ...
                                            for fii in range(0,len(filePPP)):
                                                ...
                                                for R in range(0,len(offset_)):                                                    
                                                    if (Prop__i == 'gra'):   
                                                        ...
                                                        data.append([Exp__i,Site__i,Hole__i,Core__i,Type__i,Sect__i,'',Offset__i,round_str(value__i, 2),'','','','','','','','','','','','','','','',Lithology__i])
                                                        #data.extend - test
                                                    ...
                                                    if (Prop__i == 'mad'):
                                                        ...
                                                        Moisturedry  = round_str(mad_value[0], 2)
                                                        Moisturewet  = round_str(mad_value[1], 2)
                                                        Bulkdensity  = round_str(mad_value[2], 2)
                                                        Drydensity   = round_str(mad_value[3], 2)
                                                        Graindensity = round_str(mad_value[4], 2)
                                                        Porosity     = round_str(mad_value[5], 2)
                                                    ...
                                                    if (Prop__i == 'ms'):
                                                        ...
                                                        Magneticsusceptibility = round_str(value__i, 2)
                                                    ...
                                                    if (Prop__i == 'ngr'):
                                                        ...
                                                        NGRtotalcounts = round_str(value__i, 2)
                                                    ...
                                                    if (Prop__i == 'pwl'):
                                                        ...
                                                        Pwavevelocity = round_str(value__i, 2)
                                                    ...
                                                    if (Prop__i == 'rgb'):
                                                        ...
                                                        R = round_str(rgb_value[0], 2)
                                                        G = round_str(rgb_value[1], 2)
                                                        B = round_str(rgb_value[2], 2)
                                                    ...
                                                    if (Prop__i == 'rsc'):
                                                        ...
                                                        ReflectanceL = round_str(rsc_value[0], 2)
                                                        Reflectancea = round_str(rsc_value[1], 2)
                                                        Reflectanceb = round_str(rsc_value[2], 2)
