...
def export_interp_and_csv(data,name_csv_export):
    ...
    for fc in range(0,len(df3)):
        ...
        for fcc in range(0,len(cols_alter)):
            ...
            if ( (cols_alter[fcc] == 'value_1_rsc') or (cols_alter[fcc] == 'value_2_rsc') or (cols_alter[fcc] == 'value_3_rsc')):
                value_c       = str(df3[cols_alter[fcc]][fc])
                value_ = value_c.split('.')
                if (len(value_) > 2):
                    v2 = value_[1]
                    new_value = value_[0] + "." + v2[0:2]
                    df3[cols_alter[fcc]][fc] = new_value              
                    
                    
            if ((cols_alter[fcc] != 'value_1_rgb') or (cols_alter[fcc] != 'value_2_rgb') or (cols_alter[fcc] != 'value_3_rgb')) and (df3["Core"][fc] != 11):
                value_c       = str(df3[cols_alter[fcc]][fc])
                value_ = value_c.split('.')
                if (len(value_) > 2):
                    v2 = value_[1]
            ...
            if (cols_alter[fcc] == 'value_1_ngr') and (df3["Core"][fc] == 2):
                value_c       = str(df3[cols_alter[fcc]][fc])
                value_ = value_c.split('.')
                if (len(value_[0]) > 7):
                    v2 = value_[0]
                    v2 = v2[0:5]
                    df3[cols_alter[fcc]][fc] = float(v2)
                    
            if (cols_alter[fcc] == 'value_1_pwl') and (df3["Core"][fc] == 2):
                value_c       = str(df3[cols_alter[fcc]][fc])
                value_ = value_c.split('.')
                if (len(value_[0]) > 5):
                    v2 = value_[0]
    ...
    for u in range (0,len(method_inter)):
        if (u==0):
            ...
            df_import2 = df3.interpolate(method='polynomial', order=u, limit_direction ='both')
            name_file = name_file1+"_"+method_inter[u] +"_order_"+str(u)
            ...
            df_import2 = df3.interpolate(method='spline', order=u-4, limit_direction ='both')    
            name_file = name_file1+"_"+method_inter[u] +"_order_"+str(u-4)
...
site = ['U1480']
hole = ['E']
core = np.arange(start=9, stop=13, step=1) #remove core 8: not used
typee = ['H']
    ...
    for xx in range(0,len(site)):
        for xxx in range(0,len(hole)):
            for xxxx in range(0,len(core)):
                ...
                if (len(data) > 0):                    
                    name_csv_export = str(exp[x])+'_'+str(site[xx])+'_'+str(hole[xxx])+'_'+str(core[xxxx])+''+str(typee[0])                   
