...
class Exec(wx.Frame):
    ...
    def __init__(self, parent, id=-1, title='Create dataset U1480 - wait to finish',
        ...
        p = subprocess.Popen(["python", "-u", "join_interpolation_dataset1_U1481.py"], stdout=subprocess.PIPE, bufsize=-1)
        self.pid = p.pid
        ...
        p = subprocess.Popen(["python", "-u", "create_dataset2_U1481.py"], stdout=subprocess.PIPE, bufsize=-1)
        self.pid = p.pid
        while p.poll() is None:
            x = p.stdout.readline().decode() 
            self.text1.write(x)
