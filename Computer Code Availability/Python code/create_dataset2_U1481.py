...
path                  = config.directory_IODP_images_U1481
patch_out             = config.directory_output_images_U1481
...
for folders_litho in os.listdir(patch_out):
    if (folders_litho == 'Ash_tuff'):  #to perform all lithologies, remove or adjust if ()
        ...
        for files_img_litho in os.listdir(patch_out+"/"+folders_litho):
            ...
            with open(config.file_json_U1481) as json_file: 
            ...
            for ii in range (0,len(info_full_cm_area)):
                ...
                filecsv = pd.read_csv(dataset1_U1481_group1+"dataset1_U1481_group1.csv",sep=",")
                for fi in range(0,len(filecsv)):
                    
                    Exp_    = str(filecsv['Exp'][fi])

                    Site   = str(filecsv['Site'][fi])
                    Hole   = str(filecsv['Hole'][fi])
                    SiteHole_ = Site + Hole

                    Core   = str(filecsv['Core'][fi])
